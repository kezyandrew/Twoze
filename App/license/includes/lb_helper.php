...
class LicenseBoxAPI{
	...
	public function __construct(){ 
		$this->product_id = 'C7C2F359';
		$this->api_url = 'https://ondemand.licencefly.com/';
		$this->api_key = 'CE227918E6A031-laundrysingl';
	...
	public function verify_license($time_based_check = false, $license = false, $client = false){
		...
		if($time_based_check && $this->verification_period > 0){
			...
			$today = date('d-m-Y');
			if(empty($_SESSION["61ecf44030734f6"])){
				$_SESSION["61ecf44030734f6"] = '00-00-0000';
			}
			if($type == 1){
				$type_text = '1 day';
			}elseif($type == 3){
			...
			if(strtotime($today) >= strtotime($_SESSION["61ecf44030734f6"])){
				$get_data = $this->call_api(
					'POST',
